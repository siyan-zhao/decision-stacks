...
class RewardMLP(nn.Module):
    def __init__(self, observation_dim, reward_dim=1, goal_dim=1, seq_len=100, hidden_dim=256, dropout_prob=0.1):
        super(RewardMLP, self).__init__()

        self.input_size = observation_dim * seq_len + goal_dim
        self.output_dim = reward_dim * seq_len
    ...
    def forward(self, x, goal):
        ...
        # concatenate the input sequence with the goal vector
        x_goal = torch.cat([x, goal], dim=-1)

        # predict the reward for each frame in the sequence using the fully connected layers
        reward = self.fc_layers(x_goal)

        # reshape the output sequence to match the input shape
...
class RewardTransformerEncDec(nn.Module):
    def __init__(self, observe_dim, reward_dim, hidden_size=128, num_layers=3, num_heads=1, dropout=0.1, max_ep_len=1500):
        super().__init__()
        # the max ep len is set to 1500 for time embedding.
