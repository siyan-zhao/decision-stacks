...
from .reward_models import RewardTransformerEncDec
from .action_models import ActionTransformerEncDec
from .state_models import StateTransformer
    ...
    def __init__(self,  Config, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        if self.reward_model_name == 'transformer':
            ...
            self.reward_model = nn.Sequential(
                            nn.Linear(self.observation_dim+1, hidden_dim),
                            nn.ReLU(),
        ...
        if self.action_model_name == 'transformer':
            ...
            self.action_model = nn.Sequential(
                    nn.Linear(2 * self.observation_dim+1, hidden_dim),
                    nn.ReLU(),
                    nn.Linear(hidden_dim, hidden_dim),
                    nn.ReLU(),
        ...
        if self.state_model_name == 'transformer':
            ...
            model_config = utils.Config(
                ...
                dim_mults=Config.dim_mults,
                returns_condition=return_condition,
                dim=Config.dim,
                condition_dropout=Config.condition_dropout,
                calc_energy=Config.calc_energy,
                device=Config.device)
    ...
    def loss(self, x, cond, returns=None, timesteps=None, rewards=None, total_reward=None):
        ...
        if self.train_state:
            if self.state_model_name == 'diffusion':
                ...
                goals = returns
                state_loss = self.state_model.loss(x_start=x[:, :, self.action_dim:], cond=cond, t=t, returns=goals)
                ...
                state_loss = F.mse_loss(predicted_states[:,:-1], target_states[:,1:])
            elif self.state_model_name == 'lstm':
        ...
        if self.train_rew:
            if self.reward_model_name == 'mlp':
                goals = returns.reshape(batch_size, 1, -1).repeat(1, observations.shape[1], 1)
                
                comb = torch.cat([observations, goals], dim=-1)
                pre_rewards = self.reward_model(comb).cuda()
        ...
        if self.train_inv:
            if self.action_model_name == 'mlp':
                ...
                goals = returns.reshape(batch_size, 1, -1).repeat(1, x_t.shape[1], 1)
                x_comb_t = torch.cat([x_t, x_t_1, goals], dim=-1)
                x_comb_t = x_comb_t.reshape(-1, 2 * self.observation_dim + 1)
