...
def apply_conditioning_actdiffusion(x, cond, action_dim):
    # this is for action diffusion conditioning 

    cond_obs = cond['cond_obs']
    cond_rew = cond['cond_rew']
    cond_act = cond['cond_act']
    obs_dim = cond_obs.shape[-1]
    rew_dim = cond_rew.shape[-1]
    act_dim = cond_act.shape[-1]
    x[:, :cond_obs.shape[1], :obs_dim] = cond_obs
    x[:, :cond_rew.shape[1], obs_dim:obs_dim+rew_dim] = cond_rew
    x[:, :cond_act.shape[1], -act_dim:] = cond_act
    return x

def apply_conditioning_rewdiffusion(x, conditions):
    # this is for reward diffusion conditioning 
    # print(x.shape, conditions.shape) torch.Size([15, 100, 10]) torch.Size([15, 11, 9])
    past_obs = conditions['cond_obs']
    past_rew = conditions['cond_rew']
    x[:,:past_obs.shape[1], :past_obs.shape[-1]] = past_obs
    if past_rew.shape[1] > 0:
        x[:,:past_rew.shape[1], -1] = past_rew.reshape(x.shape[0], -1)
