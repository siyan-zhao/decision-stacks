...
'''
acknowledgement
Following code are modified from https://github.com/kzl/decision-transformer/tree/master
'''


class TrajectoryModel(nn.Module):

    def __init__(self, state_dim, act_dim=None, max_length=None):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        if attention_mask is not None:
            ...
            #print(w.device, attention_mask.device,'?')
            w = w + attention_mask
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            attention_mask = encoder_attention_mask.to('cuda')
        else:
            query, key, value = self.c_attn(hidden_states).split(self.split_size, dim=2)

        query = self.split_heads(query).to('cuda')
        key = self.split_heads(key, k=True).to('cuda')
        value = self.split_heads(value).to('cuda')
