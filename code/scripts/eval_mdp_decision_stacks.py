...
from config.locomotion_config_decision_stacks_mdp import Config
import csv
import psutil
...
def evaluate(args, Config):
    ...
    decision_stacks_config = utils.Config(
    ...
    decision_stacks = decision_stacks_config()

    trainer = trainer_config(decision_stacks, dataset)
    ...
    orig_obs = obs
    while sum(dones) <  num_eval and t < Config.max_path_length:
        ...
        if t == 0:
            ...
            all_obs = torch.cat([true_obs[:, -obs_horizon-1:].cuda(), samples[:, :np.minimum(horizon, max(0, Config.max_path_length - t-1))]], dim=-2)
        assert all_obs[0, np.minimum(t, obs_horizon), 0] == true_obs[0, -1, 0]
        
        with torch.no_grad():

            if t > obs_horizon:
                timesteps = torch.arange(t - obs_horizon, np.minimum(t + horizon+1, Config.max_path_length)).unsqueeze(0).repeat(num_eval, 1)
            else:
            ...
            if args.reward_model_name == 'mlp':
                mlp_rew_input = torch.cat([all_obs, returns.reshape(num_eval, 1, 1).repeat(1, all_obs.shape[1], 1)], dim=-1)
                syn_rewards = trainer.ema_model.reward_model(mlp_rew_input)
                ...
                _, syn_rewards = trainer.ema_model.reward_model.generate(states=all_obs, tgt_reward=decoder_rewards, timestep=timesteps, rtg=returns.reshape(num_eval, 1), max_len=all_obs.shape[1]-decoder_rewards.shape[1])
            syn_rewards[:,:real_rewards[:, -obs_horizon:].shape[1]] = real_rewards[:, -obs_horizon:]
            if args.action_model_name == 'diffusion':
                cond = {'cond_obs': rew_input, 
                ...
                obs_comb = torch.cat([true_obs[:,-1].cuda(), samples[:,0], returns.reshape(num_eval, 1)], dim=-1)
                obs_comb = obs_comb.reshape(-1, 2*observation_dim + 1)
        ...
        path = 'eval_data/eval_mdp/'+  str(Config.dataset)[:-3]+ '/'  + 'DStacks' + '_w_' + \
        ...
        if t % 2 == 0 or t >= Config.max_path_length-2:
            ...
            print('avg score:', avg_score_this, ', timestep:', t, ', # of done envs', total_done, path)
        ...
        #     # uncomment to save eval log to csv
...
if __name__ == '__main__':
    ...
    parser.add_argument("--wor", action='store_true', default=False)
    parser.add_argument("--inv_mlp", action='store_true', default=False)
    parser.add_argument("--statefolder", default="decision_stacks_state_transformer_pomdp", type=str)
    ...
    parser.add_argument("--obs_horizon", type=int, default=100)
    ...
    normalized_score = evaluate(args, Config)
